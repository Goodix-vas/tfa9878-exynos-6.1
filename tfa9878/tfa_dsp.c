	...
	if (need_cal) {
		...
		case 0x94:
			snprintf(reg_state + strlen(reg_state),
				STAT_LEN - strlen(reg_state),
				", LPM1MODE %d",
				TFAxx_GET_BF(tfa, LPM1MODE));
			snprintf(reg_state + strlen(reg_state),
				STAT_LEN - strlen(reg_state),
				", LNMODE %d",
				TFAxx_GET_BF(tfa, LNMODE));
			break;
		default:
			/* neither TFA987x */
			break;
...
enum tfa98xx_error tfa_set_calibration_values_once(struct tfa_device *tfa)
{
	...
	if (tfa_count_status_flag(tfa, TFA_SET_DEVICE) == 1
		...
		/* stop and bypass if already running at resending */
		if (!tfa->is_bypass && tfa->is_cold)
	...
	if ((tfa->mtpex == 0 || tfa->reset_mtpex)
		...
		while (tries < CFSTABLE_TRIES) {
			...
			pr_info("%s: dev %d - [%d] - AREFS %d, NOCLK %d, CLKS %d, AMPS %d, PLLS %d\n",
				...
				TFAxx_GET_BF(tfa, AREFS),
				TFAxx_GET_BF(tfa, NOCLK),
				TFAxx_GET_BF(tfa, CLKS),
				TFAxx_GET_BF(tfa, AMPS),
				TFAxx_GET_BF(tfa, PLLS));
	...
	enum tfa_mtp item, int value)
{
	...
	switch (item) {
	case TFA_MTP_OTC:
		ret = tfa98xx_set_mtp(tfa, (uint16_t)
...
enum tfa98xx_error tfaxx_status(struct tfa_device *tfa)
{
	...
	snprintf(reg_state + strlen(reg_state),
		STAT_LEN - strlen(reg_state), ", LP0 %d", state);
	...
	case 0x94:
		...
		if ((control == 0x0 || control == 0x3)
			&& (state == 0x1))
			low_power = 1;
		state = TFAxx_GET_BF(tfa, LA);
		snprintf(reg_state + strlen(reg_state),
			STAT_LEN - strlen(reg_state),
			", LA %d", state);
		control = TFAxx_GET_BF(tfa, LNMODE);
		if ((control == 0x0)
			&& (state == 0x1))
			low_noise = 1;
		break;
	...
	value = TFAxx_READ_REG(tfa, OVDS); /* STATUS_FLAGS3 */
	if (value < 0)
		return -value;
	val = (uint16_t)value;

	if (!TFAxx_GET_BF_VALUE(tfa, OVDS, val))
...
enum tfa98xx_error tfa_update_log(void)
{
	...
	uint8_t cmd_buf[OFFSET_BLACKBOX_MAX * MAX_HANDLES * 3] = {0};
	int data[MAX_HANDLES * OFFSET_BLACKBOX_MAX] = {0};
	int read_size;
	...
	read_size = OFFSET_BLACKBOX_MAX * ndev * 3;
	...
	for (idx = 0; idx < ndev; idx++) {
		...
		offset = channel * OFFSET_BLACKBOX_MAX;
		group = idx * ID_BLACKBOX_MAX;

		pr_info("%s: dev %d - raw blackbox: [X = 0x%08x, T = 0x%08x]\n",
			__func__, idx,
			data[offset + OFFSET_MAXX_LOG],
			data[offset + OFFSET_MAXT_LOG]);

		/* maximum x (um) */
		data[offset + OFFSET_MAXX_LOG] = (int)
			((unsigned int)data[offset + OFFSET_MAXX_LOG]
		...
		/* maximum t (degC) */
		data[offset + OFFSET_MAXT_LOG] = (int)
			(data[offset + OFFSET_MAXT_LOG]
		...
		tfa->log_data[group + ID_OVERXMAX_COUNT]
			+= data[offset + OFFSET_OVERXMAX_COUNT];

		/* counter of t > t_max */
		tfa->log_data[group + ID_OVERTMAX_COUNT]
			+= data[offset + OFFSET_OVERTMAX_COUNT];

		pr_info("%s: dev %d - blackbox update: [%d, %d, %d, %d]\n",
			__func__, idx,
			data[offset + OFFSET_MAXX_LOG],
			data[offset + OFFSET_MAXT_LOG],
			data[offset + OFFSET_OVERXMAX_COUNT],
